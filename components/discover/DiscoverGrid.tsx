...
import { Search, MapPin, Play, Bookmark } from 'lucide-react';
...
export const DiscoverGrid: React.FC = () => {
  ...
  const cityName = gridCityFilter === 'all' ? 'Všechna města' : gridCityFilter.toUpperCase();
    ...
    <div className="min-h-screen bg-[#0A0B0E] text-white pb-32 pt-10 px-4 animate-fade-in">
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-4 py-2.5 rounded-full bg-white/10 border border-white/10 text-xs font-bold text-white shrink-0 hover:bg-white/20 active:scale-95 transition-all cursor-pointer"
        >
            ...
            <div className="relative w-full h-[220px] rounded-2xl overflow-hidden mb-2 border border-white/10">
              ...
              <div className="absolute top-3 left-3 right-3 flex items-center justify-between pointer-events-none">
                ...
                <button
                  ...
                  aria-label="Save"
                >
                  <Bookmark className={`w-4 h-4 ${savedEventIds.includes(ev.id) ? 'fill-red-500 text-red-500' : 'text-white'}`} />
                </button>
              </div>
            </div>
